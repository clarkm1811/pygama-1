...
def load_datagroup():
    ...
    # -- SURF mode -- 
    dg = DataGroup('/global/homes/c/clarkm18/SURFchar/pygama/experiments/surf/SURFCHAR.json')
    dg.load_df('/global/homes/c/clarkm18/SURFchar/pygama/experiments/surf/SURFCHAR_fileDB.h5')
...
def d2r(dg, overwrite=False, nwfs=None, vrb=False):
    ...
    for i, row in dg.file_keys.iterrows():
        ...
        # set up I/O paths
            ...
            daq_to_raw(f_daq, f_raw, config=dg.config, verbose=vrb,
                   n_max=nwfs, overwrite=overwrite, subrun=subrun, systems=systems)#, chans=chans)
